...
void LlamaGenerationSession::init(llama_model *model) {
    ...
    ctx_params.n_ctx = 4096;   // allow larger prompts from long transcripts
    ctx_params.n_batch = 4096; // keep batch size in sync with context
...
int LlamaGenerationSession::addMessage(const char *string) {
    ...
    // If the prompt does not fit in the current context, trim oldest tokens so generation can proceed.
    const int n_ctx = llama_n_ctx(ctx);
    if ((int) prompt_tokens.size() > n_ctx) {
        const int drop = (int) prompt_tokens.size() - (n_ctx - 8); // keep a small safety margin
        prompt_tokens.erase(prompt_tokens.begin(), prompt_tokens.begin() + drop);
        LOGi("Prompt too long (%d tokens), trimmed oldest %d tokens to fit context (%d)", (int) prompt_tokens.size() + drop, drop, n_ctx);
    }
